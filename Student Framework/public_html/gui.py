...
class MainWindow:
    def __init__(self, master=None, client=None):
        ...
        self._txtlog = tkst.ScrolledText(
            console_frame,
            height=10,
            wrap=tk.WORD,
            borderwidth=2,
        ...
        compose_frame.pack(side=tk.BOTTOM, expand=False, fill=tk.X, padx=10, pady=5)
        ...
        clear_button = tk.Button(compose_frame, text="clear", fg="red", command=self.clear)
        clear_button.pack(side=tk.LEFT)
        send_button = tk.Button(compose_frame, text="send", command=self.submit)
    ...
    def stop(self):
        """Wake the client thread and close the GUI event loop."""
